        ...
        if ($idVenta) {
            ...
            if ($request->input('data.cliente')) {
                ...
                if($request->input('data.cliente.nombre')){
                    ...
                    $cliente->registrarLog($cliente->ID, $venta->UsuarioActualizacionID, 'Nombre', $cliente->Nombre, $request->input('data.cliente.nombre'), 'Actualización de nombre');
                }
                if($request->input('data.cliente.apellidoPaterno')){
                    $updateData['ApellidoPaterno'] = $request->input('data.cliente.apellidoPaterno');
                    $cliente->registrarLog($cliente->ID, $venta->UsuarioActualizacionID, 'ApellidoPaterno', $cliente->ApellidoPaterno, $request->input('data.cliente.apellidoPaterno'), 'Actualización de apellido paterno');
                }
                if($request->input('data.cliente.apellidoMaterno')){
                    $updateData['ApellidoMaterno'] = $request->input('data.cliente.apellidoMaterno');
                    $cliente->registrarLog($cliente->ID, $venta->UsuarioActualizacionID, 'ApellidoMaterno', $cliente->ApellidoMaterno, $request->input('data.cliente.apellidoMaterno'), 'Actualización de apellido materno');
                }
                if($request->input('data.cliente.rut')){
                    $updateData['Rut'] = $request->input('data.cliente.rut');
                    $cliente->registrarLog($cliente->ID, $venta->UsuarioActualizacionID, 'Rut', $cliente->Rut, $request->input('data.cliente.rut'), 'Actualización de rut');
                }
                if($request->input('data.cliente.email')){
                    $updateData['Email'] = $request->input('data.cliente.email');
                    $cliente->registrarLog($cliente->ID, $venta->UsuarioActualizacionID, 'Email', $cliente->Email, $request->input('data.cliente.email'), 'Actualización de email');
                }
                if($request->input('data.cliente.telefono')){
                    $updateData['Telefono'] = $request->input('data.cliente.telefono');
                    $cliente->registrarLog($cliente->ID, $venta->UsuarioActualizacionID, 'Telefono', $cliente->Telefono, $request->input('data.cliente.telefono'), 'Actualización de teléfono');
                }
